...
- **프로젝트 분석 시 docs는 출발점, code가 근거**: repo `docs/`로 지도를 잡되, 결론은 반드시 코드·설정 파일(디렉토리 트리, go.work, Cargo.toml, Helm values, package.json)로 검증할 것. 문서 내용을 그대로 사실로 옮기지 말 것
- **문서의 작성 기준 먼저 확인**: `git log -1`로 체크아웃 시점을, `git for-each-ref --sort=-committerdate refs/remotes/origin | head`로 활성 브랜치를 확인할 것. 기본 브랜치가 활성 브랜치가 아닌 경우가 있고(main은 죽고 develop이 활성), 문서가 자기 스테일니스를 모르는 경우도 있다. git 추적 여부(`git status`)로 팀 공식 문서와 로컬 생성물을 구분할 것
