# Code Pipeline — 자체 완결 파이프라인 오케스트레이터

Brainstorming 에서 생성한 plan 디렉토리를 받아, `_dag.yaml` 에 정의된 sub-task 들을 **위상정렬 순서**로 `pre → impl → post → (fix) → clean` 파이프라인에 태웁니다.

> **전제**: 입력 디렉토리는 `DESIGN.md`, `_dag.yaml`, `NN-*.md` 를 포함해야 합니다.
...
이 문서는 얇은 오케스트레이션만 담고, 세부 로직은 같은 `references/` 에 있습니다. **필요한 시점에만** Read 로 불러오세요:

| 시점                            | 파일              |
| :------------------------------ | :---------------- |
| DAG 파싱/위상정렬               | `dag-format.md`   |
| Stage 1 (분석 + 계획) 수행 방법 | `stage-pre.md`    |
| Stage 2 (병렬 구현) 수행 방법   | `stage-impl.md`   |
| Stage 3 (종합 검증) 수행 방법   | `stage-post.md`   |
| Stage 4 (자동 수정) 수행 방법   | `stage-fix.md`    |
| Stage 5 (코드 정리) 수행 방법   | `stage-clean.md`  |
| 이슈 분류 + 중단 판단           | `halt-policy.md`  |
| fix 재시도 정책                 | `retry-policy.md` |
| 중간/최종 리포트 포맷           | `reporting.md`    |

> 참조 파일은 모두 `references/` 하위. Read 시 `references/<file>` 경로 사용.

---

## Step 0: 입력 파싱 + 검증

메인 SKILL.md 라우터에서 전달받은 plan 디렉토리 경로와 옵션을 사용:
...
4. **status 게이트**: 각 NN-<task>.md 의 frontmatter `status` 를 확인
   ...
   - `draft` 포함 시: halt + 에러
     ...
     /code <plan-dir> 로 재진입하여 편집 완료 후 status 를 ready 로 승격하세요.
     ```
